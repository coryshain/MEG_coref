...
in_3_grams = False
with open('stim_src/gigaword.3.kenlm', 'r') as f:
    for l in f:
        if not in_3_grams:
...
has_a.to_csv('stim_gen/has_a_trigram.csv', index=False)
is_a.to_csv('stim_gen/is_a_trigram.csv', index=False)
likes_to.to_csv('stim_gen/likes_to_trigram.csv', index=False)
